...
<head>
    ...
    <meta name="description" content="استعادة كلمة المرور - مرساة" />
    <link href="{{ asset('assets2/images/logo/logo.svg') }}" rel="icon" type="image/png" />
    <title>استعادة كلمة المرور | مرساة Store</title>
    ...
    <div class="w-full max-w-md my-8 space-y-6 relative z-10" data-aos="fade-up">
        ...
        <div class="text-center space-y-2">
            <a href="{{ route('guest.main-page') }}" class="inline-flex items-center gap-3">
                ...
                <div class="flex flex-col text-right">
                    ...
                    <span class="text-[10px] text-slate-400 font-semibold tracking-wider">RECOVERY PORTAL</span>
                </div>
            </a>
        </div>
        ...
        <div class="glass-card rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
            <div class="space-y-2 text-center">
                <h2 class="text-xl font-bold text-white">استعادة كلمة المرور</h2>
                <p class="text-slate-400 text-xs leading-relaxed">
                    أدخل بريدك الإلكتروني المسجل وسنرسل لك رابطاً مشفراً لتعيين كلمة مرور جديدة.
                </p>
            </div>
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-4 text-xs">
                ...
                <div class="space-y-1.5">
                    ...
                    <input id="email" name="email" type="email" :value="old('email')" required autofocus placeholder="example@email.com"
                ...
                <button type="submit" class="w-full bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 text-white font-black py-3.5 rounded-xl shadow-lg shadow-brand-600/30 transition">
                    إرسال رابط إعادة التعيين <i class="fa-solid fa-paper-plane ms-2"></i>
                </button>
            </form>

            <div class="pt-3 border-t border-slate-800 text-center">
                <a href="{{ route('login') }}" class="text-xs text-brand-400 font-bold hover:underline">
                    العودة لصفحة تسجيل الدخول <i class="fa-solid fa-arrow-left ms-1"></i>
                </a>
            </div>
        </div>
    </div>
