...
<head>
    ...
    <meta name="description" content="تأكيد كلمة المرور - مرساة" />
    <link href="{{ asset('assets2/images/logo/logo.svg') }}" rel="icon" type="image/png" />
    <title>تأكيد كلمة المرور | مرساة Store</title>
...
<body class="bg-[#0b192c] text-slate-100 min-h-screen flex flex-col justify-center items-center p-4 selection:bg-brand-500 selection:text-white">
    ...
    <div class="w-full max-w-md my-8 space-y-6 relative z-10" data-aos="fade-up">
        ...
        <div class="text-center space-y-2">
            <a href="{{ route('guest.main-page') }}" class="inline-flex items-center gap-3">
                ...
                <div class="flex flex-col text-right">
                    ...
                    <span class="text-[10px] text-slate-400 font-semibold tracking-wider">SECURITY CONFIRMATION</span>
                </div>
            </a>
        </div>
        ...
        <div class="glass-card rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
            <div class="text-center space-y-2">
                <h2 class="text-xl font-bold text-white">تأكيد كلمة المرور للمتابعة</h2>
                <p class="text-slate-400 text-xs">هذه المنطقة آمنة. يرجى تأكيد كلمة المرور الخاصة بك لمتابعة الإجراء.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.confirm') }}" class="space-y-4 text-xs">
                ...
                <div class="space-y-1.5">
                    <label for="password" class="text-slate-300 font-semibold">كلمة المرور الحالية</label>
                    <input id="password" name="password" type="password" required autofocus autocomplete="current-password" placeholder="••••••••"
                ...
                <button type="submit" class="w-full bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 text-white font-black py-3.5 rounded-xl shadow-lg shadow-brand-600/30 transition">
                    تأكيد المتابعة <i class="fa-solid fa-lock ms-2"></i>
                </button>
            </form>
        </div>
    </div>
